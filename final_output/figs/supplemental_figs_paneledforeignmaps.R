...
make_world_panel_map <- function(map_panel_data, base_map, region_type, variable, file_name, n_waste = 4, percent_scale = TRUE, ...) {
  ...
  png(glue('{fp_foreignmaps}/{file_name}.png'), height=4.5*n_waste+1+1.5,width=6.0*5+1,res=300,units='cm')
...
make_world_panel_map_vertical <- function(map_panel_data, base_map, region_type, variable, file_name, n_waste = 4, percent_scale = TRUE, ...) {
  ...
  png(glue('{fp_foreignmaps}/{file_name}.png'), width=16,height=24,res=300,units='cm')
  grid.draw(maps_laidout)
  dev.off()
